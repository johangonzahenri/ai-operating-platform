...
- **Multi-Agent Runtime Hardening: Track 4 — Official Enterprise MCP Server & Driving Adapter (`AOP-MULTIAGENT-HARDENING`)**:
  - **Official Enterprise MCP Server Driving Adapter (GAP-07)**: Implemented Model Context Protocol (MCP) driving adapter conforming to specification revision `2026-07-28` (with negotiated fallback to `2024-11-05`) in `src/platform/mcp/`. Exposes platform capabilities to external IDEs (Antigravity, VS Code, Cursor) and autonomous AI agents while strictly preserving the Core Engine hexagonal boundary (cero dependencies in Core/Domain, no direct SQLite or repository access).
  - **Methods & Capabilities**: Implemented JSON-RPC 2.0 handlers for `initialize`, `ping`, `tools/list`, `tools/call`, `resources/list`, `resources/read`, and `prompts/list`.
  - **Governance & Security Pipeline**: Direct bidirectional delegation to `ToolInvocationRuntime` enforcing RBAC authorization, agent rate limiting, team budget check, schema validation, idempotency caching, and taint boundary isolation. Connected to `HITLBridgePort` suspending sensitive/critical invocations non-blockingly (`status: "SUSPENDED_WAITING_FOR_APPROVAL"` with `resumptionToken`).
  - **MCP Transports**: Implemented Stdio transport (`serveMcpStdio`) over line-delimited readable/writable streams and HTTP transport (`handleMcpHttpRequest`) integrated into `src/platform/api/http-router.ts` at `POST /mcp`.
  ...
  - Added dedicated unit test suite `tests/unit/platform-mcp-server.test.ts` (24 tests PASS, 1836 tests passing across all 119 suites with 0 regressions, 0 skipped, 0 todo).
  - Published official architectural documentation: `docs/MCP_SERVER_ARCHITECTURE.md`, `docs/MCP_SECURITY_MODEL.md`, and `docs/MCP_CONFORMANCE_MATRIX.md`.
