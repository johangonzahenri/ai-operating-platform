...
- **Tentaciones AI Commerce: Phase 154 — On-Device Computer Vision Preprocessing & Pose/Garment Alignment Pipeline (`AOP-TENTACIONES-AR-3D-AI`, `PROJ-01`)**:
  - **Coordinate Normalization & Landmark Validation**: Implemented `normalizePixelCoordinates`, `denormalizeToPixels`, and `validateLandmark` in `src/domain/vto/pose-types.ts` supporting standard 33 canonical body landmarks (`CanonicalLandmarkIndex`), deterministic coordinate space transitions, and `UNKNOWN ≠ 0` policy preventing invalid zero-coordinate injection on occluded points.
  - **One-Euro Adaptive Temporal Low-Pass Filter**: Created pure mathematical filters `LowPassFilter`, `OneEuroFilter`, and `Point3DSmoother` in `src/domain/vto/one-euro-filter.ts` delivering low-jitter stationary stabilization ($f_{c,\min} = 1.0\,\text{Hz}$) and low-lag dynamic response ($\beta = 0.007$) with full tolerance for irregular or out-of-order timestamps.
  - **Anthropometric Ratio Calculation Engine**: Implemented `calculateAnthropometricRatios` and `calculateMidpoint` in `src/domain/vto/anthropometrics.ts` computing relative anatomical proportions (`shoulderToHipRatio`, `torsoToLegRatio`, `armSpanRatio`, `bodyInclineAngleDeg`) with explicit reliability and provenance tracking.
  - **Garment Anchoring & 2D Affine Transformation**: Developed category-specific landmark anchor resolver `resolveAnchorPosition` and 2D affine transformation engine `computeGarmentAlignment` in `src/domain/vto/garment-alignment.ts` producing rotation, scale, translation offsets, and alignment quality scores (`EXCELLENT`, `ACCEPTABLE`, `POOR`, `INSUFFICIENT_DATA`).
  - **Pose Preprocessing Pipeline & E2E Golden Journey**: Implemented `PosePreprocessingPipeline` in `src/domain/vto/pose-preprocessing-pipeline.ts` assembling `PreparedVirtualTryOnInput` and verified end-to-end inference flow.
  - Added comprehensive test suite `tests/unit/vto-pose-alignment.test.ts` (15 tests PASS, 1892 tests passing across all 140 suites with 0 failures, 0 regressions).
  - Documented in `docs/VTO_POSE_ALIGNMENT_PHASE_154.md`.
