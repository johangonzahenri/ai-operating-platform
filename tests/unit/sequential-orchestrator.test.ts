...
test("orchestrator executes model then tool sequentially and propagates bound output", async () => {
  const events = new InMemoryEventPublisher(); const result = await new SequentialOrchestrator(model, tools(), events).execute({ execution: context, operations: [
...
test("orchestrator stops after failure and reports cancelled work before it starts", async () => {
  const events = new InMemoryEventPublisher(); const failed = await new SequentialOrchestrator({ generate: async () => { throw new Error("offline"); } }, tools(), events).execute({ execution: context, operations: [
  ...
  const cancelled = await new SequentialOrchestrator(model, tools(), new InMemoryEventPublisher()).execute({ execution: context, operations: [{ kind: "MODEL", id: "x", model: "stub", input: { x: 1 } }], cancelled: true }); assert.equal(cancelled.status, "CANCELLED");
});
test("orchestrator rejects empty and duplicate operation definitions", async () => {
  const orchestrator = new SequentialOrchestrator(model, tools(), new InMemoryEventPublisher());
