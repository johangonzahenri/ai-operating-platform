...
test("runtime completes task and execution through the model strategy with correlated events", async () => {
  ...
  const runtime = new CoreRuntime(tasks, executions, new ModelExecutionStrategy(new StubModelGateway(), events), events, ids, () => new Date("2026-01-02T00:00:00Z"));
...
test("provider failure fails and persists task and execution with correlated error events", async () => {
  ...
  const runtime = new CoreRuntime(tasks, executions, new ModelExecutionStrategy(unavailable, events), events, ids);
...
test("repository infrastructure failure is propagated without changing domain state", async () => {
  ...
  const runtime = new CoreRuntime(tasks, failingExecutions, new ModelExecutionStrategy(new StubModelGateway(), events), events, ids);
