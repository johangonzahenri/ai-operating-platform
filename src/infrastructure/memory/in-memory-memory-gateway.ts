...
export class InMemoryMemoryGateway implements MemoryGateway {
  ...
  async store(item: MemoryItem): Promise<MemoryItem> { if (!item?.scope || !item.key) throw new MemoryValidationError("Memory item requires scope and key"); const stored = Object.freeze({ ...item, createdAt: new Date(item.createdAt), updatedAt: new Date(item.updatedAt) }); this.items.set(this.id(item.scope, item.key), stored); return stored; }
