  ...
  async generate(request: ModelRequest): Promise<ModelResponse> {
    ...
    try {
      response = await this.fetchFn(`${baseUrl}/v1/messages`, {
        ...
        body: JSON.stringify({
          model, max_tokens: request.maxTokens ?? 2048, temperature: request.temperature,
          system: request.systemInstruction,
          ...(request.tools ? { tools: request.tools.map((tool) => ({ name: tool.name, description: tool.description, input_schema: toJsonSchema(tool.inputSchema) })) } : {}),
          messages: request.messages
            ? request.messages.filter((item) => item.role !== "system").map((item) => item.role === "tool"
              ? { role: "user", content: [{ type: "tool_result", tool_use_id: item.toolResult.toolCallId, content: JSON.stringify(item.toolResult.output) }] }
              : { role: item.role, content: item.role === "assistant"
                ? [...(item.content ? [{ type: "text", text: item.content }] : []), ...(item.toolCalls ?? []).map((call) => ({ type: "tool_use", id: call.id, name: call.name, input: call.arguments }))]
                : item.content })
      ...
      if (error instanceof Error && error.name === "AbortError") throw new ModelTimeoutError(this.provider, `Anthropic request timed out after ${timeoutMs}ms`);
    ...
    const toolCalls = contentBlocks.filter((block) => (block as Record<string, unknown>).type === "tool_use").map((block) => {
      const item = block as Record<string, unknown>;
      return { id: String(item.id ?? ""), name: String(item.name ?? ""), arguments: (item.input && typeof item.input === "object" ? item.input : {}) as Record<string, unknown> };
    });
    const content = contentBlocks.map((block) => typeof (block as Record<string, unknown>).text === "string" ? (block as Record<string, unknown>).text : "").join("");
    ...
    if (request.requestedFormat === "json_object" || request.requestedFormat === "json_schema") {
      try { output = JSON.parse(content) as Record<string, unknown>; } catch { throw new ModelInvalidResponseError(this.provider, "Anthropic returned invalid structured JSON"); }
    }
    const usage = body.usage as Record<string, unknown> | undefined;
    const inputTokens = typeof usage?.input_tokens === "number" ? usage.input_tokens : 0;
    const outputTokens = typeof usage?.output_tokens === "number" ? usage.output_tokens : 0;
    return { provider: this.provider, model, content, output, toolCalls, usage: { inputTokens, outputTokens, totalTokens: inputTokens + outputTokens }, metadata: { provider: this.provider, model }, finishReason: typeof body.stop_reason === "string" ? body.stop_reason : "stop" };
  }
}
