...
export interface ModelOperation { readonly kind: "MODEL"; readonly id: string; readonly model: string; readonly input: Readonly<Record<string, unknown>>; readonly bindings?: readonly OperationBinding[]; readonly metadata?: Readonly<Record<string, unknown>>; }
export interface ToolOperation { readonly kind: "TOOL"; readonly id: string; readonly toolId: string; readonly input: Readonly<Record<string, unknown>>; readonly bindings?: readonly OperationBinding[]; readonly metadata?: Readonly<Record<string, unknown>>; }
export interface OrchestrationRequest { readonly execution: ExecutionContext; readonly operations: readonly Operation[]; readonly metadata?: Readonly<Record<string, unknown>>; readonly cancelled?: boolean; }
