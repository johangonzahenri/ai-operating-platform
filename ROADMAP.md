...
| **v1.4.0 Production Hardening, Developer Platform, Real-Time SSE & Reference Consumer** | AbortSignal propagation (OAD-001), persistent RBAC, authorization trace, budget/cost separation, OpenAPI 3.1, Real-Time SSE Event Streaming (`/events/stream`), Application Factory CLI (`create-aop-app`), Application Integration Certification & Reference Consumer (`examples/reference-consumer/`), failure injection tests, security controls, operations runbooks, platform narrative. | 1656 tests PASS (0 FAIL), 74 suites. | ✅ **Completed (Baseline)** |
