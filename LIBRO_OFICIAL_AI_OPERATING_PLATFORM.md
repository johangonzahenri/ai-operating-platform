...
# Capítulo 3: Las Cuatro Infografías Maestras de la Plataforma

Las cuatro infografías representan visualmente las garantías operacionales de la plataforma.

## 3.1 Blueprint 1: Topología Hexagonal en 5 Capas
![Blueprint 1: Topología Hexagonal en 5 Capas](docs/images/01_mapa_arquitectura_hexagonal.jpg)

Representa la estricta separación de fronteras:
* Los clientes externos (incluyendo aplicaciones de venta de vestuario o comercio electrónico) acceden exclusivamente a través del perímetro HTTP.
* Ningún componente externo puede invocar métodos mutadores del dominio sin pasar por los casos de uso y la barrera de políticas.

## 3.2 Blueprint 2: Flujo de Ejecución End-to-End & Trace Lifecycle
![Blueprint 2: Flujo de Ejecución End-to-End](docs/images/02_mapa_flujo_ejecucion.jpg)

Muestra la secuencia de 7 fases a través de la cual cualquier solicitud se transforma en un `ExecutionContext` inmutable portador de la tripleta `{ traceId, taskId, executionId }`, garantizando trazabilidad de punta a punta.

## 3.3 Blueprint 3: Arquitectura de Agentes de Primera Clase
![Blueprint 3: Arquitectura de Agentes](docs/images/03_mapa_arquitectura_agentes.jpg)

Consagra el invariante: **"El Agente NO reemplaza a la Ejecución"**. Un agente define la configuración declarativa de capacidad (modelo, instrucciones, herramientas permitidas y ámbito de memoria), pero su despacho se canaliza a través del único `CoreRuntime`.

## 3.4 Blueprint 4: Gobernanza Fail-Closed & Observabilidad Inmutable
![Blueprint 4: Gobernanza Fail-Closed y Observabilidad](docs/images/04_mapa_gobernanza_observabilidad.jpg)

Visualiza el mecanismo de seguridad perimetral fail-closed: si una política retorna `DENY` o el motor evaluador experimenta una falla, la ejecución se interrumpe de inmediato. En paralelo, los eventos se difunden a suscriptores pasivos de telemetría y auditoría sin alterar el flujo de inferencia.
